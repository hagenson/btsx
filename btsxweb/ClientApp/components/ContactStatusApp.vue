<template>
    <div class="container mt-4">
        <div v-if="jobNotFound" class="text-center mt-5">
            ...
            <p class="text-muted">No Contact Transfer Job with the specified ID was found.</p>
        </div>

        <div v-else>
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="mb-0">Contact Transfer Status</h1>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div v-show="showStatistics" class="mt-3">
                            ...
                            <table class="table table-sm">
                                <tbody>
                                    ...
                                    <tr>
                                        <td><strong>Successfully Transferred:</strong></td>
                        ...
                        <div v-if="!isJobCompleted" class="mt-3">
                            <button class="btn btn-danger" @click="cancelTransfer" :disabled="isCancelling">Cancel</button>
                        </div>
...
<script setup lang="ts">
        ...
        try {
            ...
            if (info) {
                ...
                if (info.isCompleted) {
                    ...
                    if (info.totalContacts || info.successfulContacts || info.skippedContacts || info.failedContacts) {
                        showStatistics.value = true;
                        statistics.value = {
                            total: info.totalContacts,
                            success: info.successfulContacts,
                            skipped: info.skippedContacts,
                            failed: info.failedContacts
                        };
                    }
                }
            } else {
    ...
    onMounted(async () => {
        props.connection.on("StatusUpdate", (job: JobInfo) => {
            ...
            if (job.totalContacts) {
                showStatistics.value = true;
                statistics.value = {
                    total: job.totalContacts,
                    success: job.successfulContacts || 0,
                    skipped: job.skippedContacts || 0,
                    failed: job.failedContacts || 0
                };
            }
        });
    });
    ...
    function cancelTransfer() {
        feedbackModal.value?.prompt({
            ...
            message: 'Are you sure you want to cancel this contact transfer?',
            iconClass: 'exclamation-triangle',
            yesAction: () => {
                isCancelling.value = true;
                props.connection.invoke("CancelTransfer", props.jobId)
                    .catch((err: Error) => {
    ...
    function bookmarkPage() {
        ...
        const title = jobInfo.value?.sourceServer
            ? `Contact Transfer Status - ${jobInfo.value.sourceServer}`
            : `Contact Transfer Status - Job ${props.jobId}`;
